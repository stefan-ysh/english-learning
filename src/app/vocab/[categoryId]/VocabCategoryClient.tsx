...
interface VocabCategoryClientProps {
    categoryId: string;
}

export default function VocabCategoryClient({ categoryId }: VocabCategoryClientProps) {
    const { t } = useI18n();
    ...
    return (
        <main className="flex min-h-screen flex-col items-center p-4 md:p-24 bg-gray-50 dark:bg-gray-950">
            <div className="z-10 max-w-5xl w-full flex items-center justify-between font-mono text-sm lg:flex mb-12">
                ...
                <h1 className="text-2xl font-bold">{t(`cat.${categoryId}`)}</h1>
                <div className="w-16"></div>
            </div>

            <div className="grid w-full max-w-xl gap-6">
                <Link
                    href={`/vocab/${categoryId}/learn`}
